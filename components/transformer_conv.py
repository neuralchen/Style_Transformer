...
class DeepEmbed(nn.Module):
    def __init__(self, indim=256, outdim=256):
        super().__init__()
        self.layer1 = nn.Conv2d(indim, indim, 3,1,1)
        self.activation = nn.ReLU()
        self.layer2 = nn.Conv2d(indim, outdim, 1,1,0)
...
class MultiHead(nn.Module):
    ...
    def forward(self, content, reference, attention_mask=None):
        ...
        c_ = self.all_head_size

        mixed_query_layer = self.query(content).view(b,c_,-1).transpose(-1,-2)
        mixed_key_layer = self.key(reference).view(b,c_,-1).transpose(-1,-2)
        mixed_value_layer = self.value(reference).view(b,c_,-1).transpose(-1,-2)  ### b * wh * dim
